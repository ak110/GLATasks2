...
class List(Base):
    ...
    user_id: Mapped[int] = mapped_column(
        sqlalchemy.ForeignKey("user.id"), nullable=False, comment="親のユーザーID"
    )
    status: Mapped[str] = mapped_column(
        sqlalchemy.String(255), nullable=False, default="show", comment="ステータス"
    )
    title: Mapped[str] = mapped_column(
        sqlalchemy.String(255), nullable=False, comment="リスト名"
    )
