...
class User(Base, pytilpack.quart_auth.UserMixin):
    ...
    user: Mapped[str] = mapped_column(sqlalchemy.String(80), unique=True, nullable=False, comment="ユーザーID")
    pass_hash: Mapped[str] = mapped_column(sqlalchemy.String(255), nullable=False, comment="パスワードハッシュ")
    ...
    def set_password(self, new_password):
        ...
        self.pass_hash = bcrypt.hashpw(new_password.encode("utf-8"), salt).decode("utf-8")

    def password_is_ok(self, input_password):
        """パスワードチェック。"""
        return bcrypt.checkpw(input_password.encode("utf-8"), self.pass_hash.encode("utf-8"))
