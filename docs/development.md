...
`make test` を実行すると lint + 型チェック + ユニットテスト + e2e がまとめて実行される。
...
push 時に自動実行。lint + 型チェック + ユニットテスト（`pnpm run test`）を実行する。e2e テストは CI では実行しない（Docker Compose 環境が必要なため）。
...
sequenceDiagram
    ...
    Dev->>R: gh workflow run release.yml
    R->>T: test.yml の成功を待機
    T-->>R: 成功
    R->>R: タグ・リリース作成
    R->>D: gh workflow run deploy.yml
    D->>D: Docker イメージビルド → GHCR プッシュ
    D->>S: SSH で make sync && make deploy
```

1. `release.yml`（手動実行）: master の test.yml が成功するまで待機 → バージョンタグとリリースを作成 → `gh workflow run deploy.yml` でデプロイを起動
2. `deploy.yml`（release.yml から起動）: Docker イメージをビルドして GHCR にプッシュ → SSH でサーバーに接続し `make sync && make deploy` を実行
