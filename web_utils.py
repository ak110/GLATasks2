...
def register_csrf_token(
    app, session_key="_csrf_token", form_key="nonce", func_name="csrf_token"
):
    """CSRF対策の処理を登録する。
        ...
        if quart.request.method == "POST":
            ...
            if not token:
                quart.current_app.logger.warning(
                    f"No CSRF token in session. ({quart.current_app.root_path})"
                )
                quart.abort(403)
            if token != (form_token := (await quart.request.form).get(form_key)):
                quart.current_app.logger.warning(
                    f"CSRF Token Error! ({token=} {form_token=} {quart.request.path})"
                )
                quart.abort(403)
