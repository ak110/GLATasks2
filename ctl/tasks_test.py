...
async def test_task_operations(user_client: quart.typing.TestClientProtocol):
    ...
    nonce = await get_nonce(user_client)
    response = await user_client.post(
        f"/tasks/{list_id}", form={"text": "テストタスク", "nonce": nonce}
    )
    assert response.status_code == 302
    ...
    response = await user_client.patch(
        f"/tasks/api/{list_id}/{task_id}", json={"status": "completed"}
    )
    ...
    # テキスト更新
    response = await user_client.patch(
        f"/tasks/api/{list_id}/{task_id}", json={"text": "API経由で更新"}
    )
    assert response.status_code == 200
    ...
    response = await user_client.patch(
        f"/tasks/api/{list_id}/{task_id}", json={"completed": test_date}
    )
    assert response.status_code == 200
    data = json.loads(await response.get_data())
...
async def test_access_control(user_client: quart.typing.TestClientProtocol):
    ...
    nonce = await get_nonce(user_client)
    response = await user_client.post(
        "/tasks/99999", form={"text": "テスト", "nonce": nonce}
    )
    ...
    nonce = await get_nonce(user_client)
    response = await user_client.post(
        "/tasks/patch/1/99999", form={"text": "テスト", "nonce": nonce}
    )
    assert response.status_code == 404

    response = await user_client.patch(
        "/tasks/api/1/99999", json={"status": "completed"}
    )
    assert response.status_code == 404
